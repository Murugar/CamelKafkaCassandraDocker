package com.iqmsoft.kafka.cassandra.scala
...
object Main extends App {
  ...
  // while true;do kafka-console-producer -brokers localhost:9092 -topic "camelkafkacassandra" -key $RANDOM -value "{\"name\":\"foo$RANDOM\", \"value\": \"Value$RANDOM\"}";sleep 1;done;
  ...
  ctx.addRoutes(new RouteBuilder {
    override def configure() {
      from("kafka:localhost:9092?zookeeperHost=localhost&topic=camelkafkacassandra&groupId=test_camelkafkacassandra")
        ...
        .to("cql:172.17.0.1:9042/dev?username=cassandra&cql=INSERT INTO messages (k, v) VALUES(?, ?)")
    }
  })
  ctx.start()
